    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                ...
                acceptExtensions = (editor.options.limit || editor.getOpt('imageAllowFiles') || []).join('').replace(/\./g, ',').replace(/^[,]/, ''),
                imageMaxSize = editor.options.limitMaxSize || editor.getOpt('imageMaxSize'),
...
//                $('.webuploader-container').show();
            ...
            uploader.on('fileDequeued', function (file) {
            	//如果格式不正确，不进行减法
            	if (!file.ext || acceptExtensions.indexOf(file.ext.toLowerCase()) == -1) {
                }else if(!file.size || imageMaxSize < file.size){
                }else{
                	fileCount--;
                    fileSize -= file.size;
                }
        ...
        getQueueCount: function () {
            ...
            if(editor.options.fileCount <= readyFile){
             ...
             //检查能添加几个文件。
//            if(readyFile >= editor.options.fileCount){
//            	//禁用继续添加和图片按钮
//            	 $('.webuploader-container').hide();
//            }else{
//            	$('.webuploader-container').show();
//            }
        ...
        getInsertList: function () {
            var i, data, list = [],
                align = getAlign(),
                //修改为动态配置basePath,basePath='/boyuyun/
